...
from infrastructure.llm.router import LLMProviderSpec, LLMRouter  # noqa: E402
...
def make_router(allowed_models=None):
    ds, oa = [], []
    specs = {
...
async def test_generate_with_selects_requested_provider():
...
async def test_generate_with_none_provider_falls_back_to_default():
    router, ds, oa = make_router()
...
async def test_model_override_creates_and_caches_client():
...
async def test_model_outside_allowlist_rejected():
...
async def test_no_allowlist_permits_any_model():
...
async def test_default_provider_must_be_among_enabled():
    specs = {
...
async def test_enabled_providers_property():
    router, _, _ = make_router()
