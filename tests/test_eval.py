...
async def test_runner_computes_all_metrics():
    responses = {
        "good": {
    ...
    report = await EvalRunner(FakeRetriever(responses), top_k=2).run(cases)
    d = report.to_dict()
...
async def test_runner_reports_errors_as_failed_cases():
    async def failing(user_id, query, top_k=None):
        raise RuntimeError("llm down")

    report = await EvalRunner(failing).run([EvalCase(query="q")])
    d = report.to_dict()
    assert d["errors"] == 1
    assert d["cases"][0]["error"] == "llm down"


async def test_compare_reports_deltas():
