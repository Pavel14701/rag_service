...
class AnthropicClient(LLMGenerator):
    ...
    async def generate(
        ...
        with get_tracer().start_as_current_span("llm.generate") as span:
            ...
            # Anthropic calls them input/output tokens; map to the shared metrics.
            if usage.get("input_tokens"):
                LLM_TOKENS_TOTAL.labels(kind="prompt").inc(usage["input_tokens"])
            if usage.get("output_tokens"):
                LLM_TOKENS_TOTAL.labels(kind="completion").inc(usage["output_tokens"])
            span.set_attribute(
                "llm.completion_tokens", usage.get("output_tokens", 0)
            )

            # The response content is a list of typed blocks; concatenate text ones.
            content = "".join(
                block.get("text", "")
